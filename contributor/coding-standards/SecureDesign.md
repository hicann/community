...
1.5 根据权限最小化原则，运行软件程序的账号要尽可能的使用操作系统低权限的账号。

**说明**：对于运行软件程序的操作系统账号，不应使用“root”、“administrator”、“supervisor”等特权账号或高级别权限账号，应该使用普通权限的账号运行。
...
## 5. 开发和发布安全
5.1 社区代码/版本(包含使用的三方件)经社区指定的漏洞扫描工具扫描，高风险级别的漏洞必须得到解决或有效规避。

**说明**：社区版本正式发布前，应使用社区指定的漏洞扫描工具对版本进行漏洞扫描测试(测试应使用当时最新的漏洞库、配置所有漏洞相关插件)。扫描结果应进行分析处理，确认漏洞的实际风险级别，高风险级别的漏洞必须在社区版本正式发布前得到解决。

5.2 社区版本禁止存在病毒、木马。

**说明**：社区版本必须使用社区指定的防病毒软件对其扫描，保证软件包中未感染或嵌入病毒/木马。如果防病毒软件产生告警，必需进行规避以消除告警。防病毒软件名称及软件版本、病毒库版本、扫描时间、扫描结果等需要存档。

5.3 禁止在社区版本中存留可能被质疑的组件，包括第三方的嗅探工具、调试工具、开发/编译工具，调试工具/脚本。

5.4 禁止破坏OS等系统原有安全框架、严重违反原有系统安全设计原则、可能被外界质疑为后门的行为。

5.5 严禁在包含AI系统与模型的解决方案与服务中植入任何形式的后门程序或代码。

## 6. 安全编码/编译

6.1 社区代码必须经过社区指定的静态代码检查工具扫描，并按照相应语言的告警清理要求进行清理。

6.2 必须按规范开启安全编译选项。

**说明**： 社区版本正式发布时，符合安全编译选项应用范围的代码，安全编译必选选项必须落地。