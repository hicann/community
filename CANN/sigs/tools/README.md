# tools SIG

### 简介

欢迎来到**tools SIG**！我们致力于构建模型压缩和问题定位领域的完整技术生态，探索极致高效的模型优化路径和精准高效的问题诊断方法！

**工具列表**
|   名称   |  领域  |  介绍  |
|----------|----------|----------|
| [amct](https://gitcode.com/cann/amct) | 模型压缩 | 通过量化等优化算法，完成模型的“瘦身”和“提速”，提升模型部署性能 |
| [oam-tools](https://gitcode.com/cann/oam-tools) | 故障定位 | 为开发者提供故障定位和性能测试调优工具，提升问题定位效率 |

### 工作目标
- 1、负责工具领域技术路线规划和落地；
- 2、组织工具领域例会，引导技术讨论和决策；
- 3、负责周边组织沟通协调，推动落地相关项目；
- 4、负责SIG仓库权限和邮件列表等基础设施的管理；
- 5、负责社区Bug、issue和邮件列表等渠道反馈的问题分发处理。


### 联系方式
两周一次例会，每单周周一下午16:00-18:00
- [会议链接](https://meeting.osinfra.cn/cann/)
- [会议议题&纪要](https://etherpad.meeting.osinfra.cn/p/sig-tools)
- [邮件订阅](https://mailweb.cann.osinfra.cn/mailman3/lists/tools.cann.osinfra.cn/)


### SIG成员

#### Maintainer列表
- 金瑛琪 [[@jinyingqi]](https://gitcode.com/jinyingqi), *jinyingqi@huawei.com*
- 姚广修 [[@yaoguangxiu]](https://gitcode.com/yaoguangxiu), *yaoguangxiu@huawei.com*
- 沃文杰 [[@wowenjie]](https://gitcode.com/wowenjie), *wowenjie@huawei.com*
- 袁江 [[@yuanjiang_]](https://gitcode.com/yuanjiang_), *yuanjiang@huawei.com*

#### Committer列表
- 傅珺 [[@fujun19]](https://gitcode.com/fujun19), *fujun19@hisilicon.com*
- 朱梁英 [[@zhuliangying]](https://gitcode.com/zhuliangying), *zhuliangying@huawei.com*
- 吴林玉 [[@wulinyu]](https://gitcode.com/wulinyu), *wulinyu4@huawei.com*
- 李鹏程 [[@lipengcheng30]](https://gitcode.com/lipengcheng30), *lipengcheng30@hisilicon.com*
- 焦同辉 [[@nice_123]](https://gitcode.com/nice_123), *jiaotonghui1@hisilicon.com*
- 代家印 [[@daijiayin]](https://gitcode.com/daijiayin), *daijiayin@hisilicon.com*
- 梁横溢 [[@lianghengyi]](https://gitcode.com/lianghengyi), *lianghengyi1@huawei.com*
- 陈卓 [[@zchenbm97]](https://gitcode.com/zchenbm97), *chenzhuo63@hisilicon.com*
- 李艇 [[@li_ting]](https://gitcode.com/li_ting), *liting73@hisilicon.com*
- 汪凯 [[@wk0911]](https://gitcode.com/wk0911), *wangkai134@huawei.com*
- 陈豪 [[@chenhao_1209]](https://gitcode.com/chenhao_1209), *chenhao61@huawei.com*
- 张杰 [[@newstarzj]](https://gitcode.com/newstarzj), *zhangjie230@huawei.com*

