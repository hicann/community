...
## 介绍
CANN社区有技术指导委员会和SIG组，SIG成员和仓库管理需要规范管理。
...
角色|	字段|	权限范围|
|------|----------|----------|
|导师（Mentor）|	mentors	|SIG战略指导（无PR权限）|
|审批者（Committer）|	committers |	代码合并权限（可覆盖到文件级）|
|审查者（Reviewer）|	reviewers	|代码评审权限（可覆盖到文件级）|
...
| mentors | 列表 |一层|  SIG组当前导师名单(SIG组导师负责明确SIG组职责、愿景，促进SIG组内部沟通协作等) |
| committers| 列表 | 一层|SIG组对应的committer名单 |
| reviewers| 列表 | 一层|SIG组对应的reviewer名单 |
...
| contributors | 列表 |二层 | 仓库组对应的contributor名单 ，可选|
...
- gitcode_id: ccc
- gitcode_id: ddd
reviewers:                            #SIG组所有reviewer名单 （eee和fff拥有MindI sig组下所有仓库的reviewer权限）
- gitcode_id: eee
- gitcode_id: fff
...
- gitcode_id: ccc
- gitcode_id: ddd
reviewers:                            #SIG组所有reviewer名单 （eee和fff拥有MindI sig组下除了ops-basic-AAA、ops-basic-BBB仓库以外的其它仓库的reviewer权限）
- gitcode_id: eee
- gitcode_id: fff
repositories:                         #repositories 字段会说明SIG组所管理的仓库组的信息
- repo:                               #仓库组，可以是一个仓库，也可以是一组仓库 ，可选字段
  - CANN/ops-basic-AAA
  - CANN/ops-basic-BBB
  committers:                         #仓库组对应的committer名单 ，可选字段 （ggg和hhh拥有ops-basic-AAA、ops-basic-BBB仓库的committer权限）
  - gitcode_id: ggg
  - gitcode_id: hhh
  reviewers:                          #仓库组对应的reviewer名单 ，可选字段 （iii和jjj拥有ops-basic-AAA、ops-basic-BBB仓库的reviewer权限）
  - gitcode_id: iii
  - gitcode_id: jjj
- repo:                               #仓库组，可以是一个仓库，也可以是一组仓库 ，可选字段
  - CANN/ops-basic-CCC
...
- repo:                               #仓库组，可以是一个仓库，也可以是一组仓库 ，可选字段
  - CANN/ops-basic-AAA
  - CANN/ops-basic-BBB
  committers:                         #仓库组对应的committer名单 ，可选字段 （ggg和hhh拥有ops-basic-AAA、ops-basic-BBB仓库下除了cmake/figures目录和siginfo.yaml文件的committer权限）
  - gitcode_id: ggg
  - gitcode_id: hhh
  reviewers:                          #仓库组对应的reviewer名单 ，可选字段 （ggg和hhh拥有ops-basic-AAA、ops-basic-BBB仓库下除了cmake/figures目录和siginfo.yaml文件的reviewer权限）
  - gitcode_id: iii
  - gitcode_id: jjj
  ...
  - path:                             #entry组，可以是一个目录或者完整的文件路径，也可以是一组目录或者完整的文件路径，可选字段
    - cmake/figures
    - siginfo.yaml
    committers:                       #entry组对应的committer名单 ，可选字段 （yyy和zzz拥有ops-basic-AAA、ops-basic-BBB仓库下cmake/figures目录和siginfo.yaml文件的committer权限）
    - gitcode_id: yyy
    - gitcode_id: zzz
    reviewers:                        #entry组对应的reviewer名单 ，可选字段 （qwe和rty拥有ops-basic-AAA、ops-basic-BBB仓库下cmake/figures目录和siginfo.yaml文件的reviewer权限）
    - gitcode_id: qwe
    - gitcode_id: rty
  - path:                             #entry组，可以是一个目录或者完整的文件路径，也可以是一组目录或者完整的文件路径，可选字段
    - debug/validate
...
- repo:                               #仓库组，可以是一个仓库，也可以是一组仓库 ，可选字段
  - CANN/ops-basic-AAA
  - CANN/ops-basic-BBB
  committers:                         #仓库组对应的committer名单 ，可选字段 （ggg和hhh拥有ops-basic-AAA、ops-basic-BBB仓库除了master、br_release分支的committer权限）
  - gitcode_id: ggg
  - gitcode_id: hhh
  reviewers:                          #仓库组对应的reviewer名单 ，可选字段 （iii和jjj拥有ops-basic-AAA、ops-basic-BBB仓库除了master、br_release分支的reviewer权限）
  - gitcode_id: iii
  - gitcode_id: jjj
  ...
  - branch:                           #分支组，可以是一个分支，也可以是一组分支，可选字段
    - master
    - br_release
    committers:                       #分支组对应的committer名单 ，可选字段 （kkk和LLL拥有ops-basic-AAA、ops-basic-BBB仓库master、br_release分支的committer权限）
    - gitcode_id: kkk
    - gitcode_id: LLL
    reviewers:                        #分支组对应的reviewer名单 ，可选字段 （mmm和nnn拥有ops-basic-AAA、ops-basic-BBB仓库master、br_release分支的reviewer权限）
    - gitcode_id: mmm
    - gitcode_id: nnn
...
- repo:                               #仓库组，可以是一个仓库，也可以是一组仓库 ，可选字段
  ...
  - branch:                           #分支组，可以是一个分支，也可以是一组分支，可选字段
    ...
    - path:                           #entry组，可以是一个目录或者完整的文件路径，也可以是一组目录或者完整的文件路径，可选字段
      - cmake/figures
      - siginfo.yaml
      committers:                     #entry组对应的committer名单 ，可选字段 （qqq和rrr拥有ops-basic-AAA、ops-basic-BBB仓库master和br_release分支下cmake/figures目录和siginfo.yaml文件的committer权限）
      - gitcode_id: qqq
      - gitcode_id: rrr
      reviewers:                      #entry组对应的reviewer名单 ，可选字段 （sss和ttt拥有ops-basic-AAA、ops-basic-BBB仓库master和br_release分支下cmake/figures目录和siginfo.yaml文件的reviewr权限）
      - gitcode_id: sss
      - gitcode_id: ttt
    - path:                           #entry组，可以是一个目录或者完整的文件路径，也可以是一组目录或者完整的文件路径，可选字段
      - debug/validate
