...
采用 `语言A | 语言B` 形式，两种语言名称同时展示，以竖线 `|` 分隔。**当前语言显示为纯文本在前，不要链接，非当前语言按字母顺序排后面，显示为链接**，链接指向对应语言版本的文件：

| 文件 | 按钮显示文本 | 链接目标 |
|------|-------------|---------|
| `README.md`（中文版） | ` 简体中文\| English` | `English` 链接到 `README_en.md`，`简体中文` 为纯文本 |
...
- 整仓执行**Doc Tools**检查：在VS Code中安装**Doc Tools**插件，在要检测的子目录上右击，选择Doc Tools，依次执行以下四项检测：
